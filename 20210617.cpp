...
ATOM MyRegisterClass(HINSTANCE hInstance)//윈도우 클래스 등록
{
    ...
    wcex.style          = CS_HREDRAW | CS_VREDRAW;//가로세로 길이 변경
    ...
    case WM_PAINT://윈도우 클라이언트 영역에서 일어나는 모든일 총괄 함수,hdc 꼭 포홤시키기 
        {
            ...
            SetTextColor(hdc, RGB(150, 0, 200));//문자색 설정
            SetBkColor(hdc, RGB(0, 0, 255));//문자 배경색 설정
            //TextOut(hdc, 200, 200, TEXT("HAMELLEN"), 8);//TEXT의 문자수와 길이가 맞지 않으면 여부 문자 출력,배치 관리자 미적용 
            //SetTextAlign(hdc,TA_BOTTOM | TA_RIGHT); 다음문자정렬 위치 설정
            RECT rt;//RECT 구조체 변수 저장
            //GetClientRect(hWnd, &rt);//윈도우의 클라이언트 영역의 사각형 정보 rt에 저장
            //DrawTextW(hdc, TEXT("안녕"), -1, &rt, DT_CENTER);//배치관리자 적용
            //for (int x = 100; x <= 300; x++)
                //for (int y = 100; y <= 400; y++)
                    //SetPixel(hdc, x, y, RGB(150, 0, 200));픽셀 연습본,비효율적
            //LineTo(hdc,100,100) 디폴트 죄표(0,0)에서 (100,100)까지 직선을 귿는다. 변경된 좌표는 저장된다. 
            //MoveToEx(hdc,200,200,NULL) 변경된 좌표에서 (200,200)으로 좌표이동 
            //Rectangle(hdc,100,300,300,500); 직사각형 출력
            //Ellipse(hdc,100,200,200,400); 타원 출력
